...
const searchSong = (songUrl, index) => {
  fetch(`https://shazam.p.rapidapi.com/search?term=${songUrl}&locale=en-US&offset=0&limit=5`, optionGet)
    .then(response => response.json())
    .then(response => {
      searchInputEL.append(response.tracks.hits[index || 0].track.title);
      inputDescriptionEL.append('The artist/band for this song is: ' + response.tracks.hits[index || 0].track.subtitle);
      playerEL.innerHTML = `<a href=" ` + response.tracks.hits[index || 0].track.url + `" target="_blank">View this track on Shazam!</>`;
...
const songReccomendation = (songId) => {
  fetch('https://shazam.p.rapidapi.com/songs/list-recommendations?key='+ songId +'&locale=en-US', optionGet)
  .then(response => response.json())
  .then(response => {
    if(!response.tracks){
      relatedItem1EL.append('No reccomendations found')
      relatedItem2EL.append('No reccomendations found')
      relatedItem3EL.append('No reccomendations found')
    }
    else{
      relatedItem1EL.innerHTML = 'Similar Song: '+ '<a href="'+ response.tracks[0].url + '" target="_blank">' + response.tracks[0].title + ' by ' + response.tracks[0].subtitle + '</a>';
      relatedItem2EL.innerHTML = 'Similar Song: '+ '<a href="'+ response.tracks[1].url + '" target="_blank">' + response.tracks[1].title + ' by ' + response.tracks[1].subtitle + '</a>';
      relatedItem3EL.innerHTML = 'Similar Song: '+ '<a href="'+ response.tracks[2].url + '" target="_blank">' + response.tracks[2].title + ' by ' + response.tracks[2].subtitle + '</a>';
    }
  })
  .catch(err => {
    if(err)throw(err)
